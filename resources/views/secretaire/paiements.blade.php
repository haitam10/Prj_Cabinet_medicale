...
<body class="bg-gray-100 min-h-screen">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        <span id="paiementCount">{{ $paiements->total() ?? 0 }} paiement{{ ($paiements->total() ?? 0) > 1 ? 's' : '' }}</span>
                    </div>
                </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200" id="paiementsTableBody">
                        @forelse($paiements as $paiement)
                            <tr class="hover:bg-gray-50 transition-colors paiement-row"
                                ...
                                data-date="{{ $paiement->date_paiement }}">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ $paiement->facture->patient->nom ?? 'N/A' }} {{ $paiement->facture->patient->prenom ?? '' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
    ...
    <div id="createPaiementModal" class="modal fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white w-full max-w-lg rounded-lg shadow-xl p-6 m-4 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-6">
                ...
                <button onclick="closeModal('createPaiementModal')" class="text-gray-400 hover:text-gray-600 transition-colors">
            ...
            <form method="POST" action="{{ route('paiements.store') }}" class="space-y-4">
                    ...
                    <select id="facture_id" name="facture_id" required
                        ...
                        @foreach($factures ?? [] as $facture)
                            <option value="{{ $facture->id }}" data-montant="{{ $facture->montant }}">
                                Facture #{{ $facture->id }} - {{ $facture->patient->nom ?? 'N/A' }} {{ $facture->patient->prenom ?? '' }} ({{ number_format($facture->montant, 2) }} DH)
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" id="date_paiement" name="date_paiement" value="{{ date('Y-m-d') }}" required readonly
    ...
    <div id="editPaiementModal" class="modal fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white w-full max-w-lg rounded-lg shadow-xl p-6 m-4 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-semibold text-gray-800">Modifier le paiement</h2>
                <button onclick="closeModal('editPaiementModal')" class="text-gray-400 hover:text-gray-600 transition-colors">
                    <i class="fas fa-times text-lg"></i>
                </button>
            </div>
            <form id="editPaiementForm" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="edit_montant" name="montant" step="0.01" min="0" required
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cordes-blue focus:border-transparent">
                    </div>
                    <div>
    ...
    <!-- Modal Voir Paiement -->
    <div id="showPaiementModal" class="modal fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white w-full max-w-lg rounded-lg shadow-xl p-6 m-4 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-semibold text-gray-800">Détails du paiement</h2>
                <button onclick="closeModal('showPaiementModal')" class="text-gray-400 hover:text-gray-600 transition-colors">
                    <i class="fas fa-times text-lg"></i>
                </button>
            </div>
            <div class="space-y-4">
    ...
    <script>
        ...
        function showPaiement(id) {
            ...
            .then(data => {
                ...
                document.getElementById('show_date').textContent = new Date(data.date_paiement).toLocaleDateString('fr-FR');
