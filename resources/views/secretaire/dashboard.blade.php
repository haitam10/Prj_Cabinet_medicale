...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="fixed inset-y-0 left-0 w-64 bg-cordes-dark shadow-xl z-50">
        ...
        <div class="absolute bottom-4 left-4 right-4">
            <div class="bg-gray-50 rounded-xl p-4 group cursor-pointer hover:bg-red-50 transition-colors duration-200">
                <form method="POST" action="{{ route('logout') }}" id="logout-form">
                    @csrf
                    <div class="flex items-center space-x-3" onclick="document.getElementById('logout-form').submit();">
                        <img src="https://cdn-icons-png.flaticon.com/512/17003/17003310.png" alt="User" class="w-10 h-10 rounded-full">
                        <div>
                            <p class="text-gray-900 text-sm font-medium">{{ Auth::user()->nom ?? 'Utilisateur' }}</p>
                            <p class="text-gray-500 text-xs">{{ ucfirst(Auth::user()->role ?? '') }} — <span class="text-red-400 group-hover:text-red-600">Se déconnecter</span></p>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Welcome back, {{ Auth::user()->nom ?? 'Mohammad' }} 👋</h1>
        </div>
        <div class="flex items-center space-x-4">
            <div class="flex items-center space-x-2">
                <select id="date-filter" class="text-sm border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
                    <option value="day" {{ $period == 'day' ? 'selected' : '' }}>Day</option>
                    <option value="month" {{ $period == 'month' ? 'selected' : '' }}>Month</option>
                    <option value="year" {{ $period == 'year' ? 'selected' : '' }}>Year</option>
                </select>
            </div>
            <button onclick="exportToPDF()" class="bg-primary text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-primary-dark transition-colors">
        ...
        <main class="p-6">
           ...
           <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <!-- Factures Card -->
                <div onclick="openFactureModal()" class="bg-gradient-to-br from-primary to-primary-dark rounded-2xl p-6 text-white cursor-pointer hover:shadow-lg transform hover:-translate-y-1 transition-all duration-200">
                    <div class="flex items-center justify-between mb-4">
                        <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
                            <i class="fas fa-file-invoice-dollar text-white text-xl"></i>
                        </div>
                        <span class="text-white/80 text-sm" id="facts-percent">{{ number_format($facts_diff_percent, 1) }}%</span>
                    </div>
                    ...
                    <div class="mt-4 bg-white/20 rounded-full h-2">
                        <div class="bg-white rounded-full h-2" style="width: {{ abs($facts_diff_percent) > 100 ? 100 : abs($facts_diff_percent) }}%"></div>
                    </div>
                    <p class="text-white/80 text-xs mt-2" id="facts-change">{{ $facts_diff_percent >= 0 ? 'Increase' : 'Decrease' }} by {{ number_format($count_facts_filtered) }} Factures</p>
                </div>

                <!-- Patients Card -->
                <div onclick="openPatientModal()" class="bg-white rounded-2xl p-6 border border-gray-100 cursor-pointer hover:shadow-lg transform hover:-translate-y-1 transition-all duration-200">
                    <div class="flex items-center justify-between mb-4">
                        <div class="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center">
                            <i class="fas fa-users text-primary text-xl"></i>
                        </div>
                        <span class="text-green-600 text-sm" id="pats-percent">{{ number_format($pats_diff_percent, 1) }}%</span>
                    </div>
                    <h3 class="text-gray-600 text-sm font-medium mb-1">Patients</h3>
                    <p class="text-3xl font-bold text-gray-900" id="pats-count">{{ number_format($count_pats_filtered) }}</p>
                    <div class="mt-4 bg-gray-200 rounded-full h-2">
                        <div class="bg-green-600 rounded-full h-2" style="width: {{ abs($pats_diff_percent) > 100 ? 100 : abs($pats_diff_percent) }}%"></div>
                    </div>
                    <p class="text-gray-500 text-xs mt-2" id="pats-change">{{ $pats_diff_percent >= 0 ? 'Increase' : 'Decrease' }} by {{ number_format($count_pats_filtered) }} Patients</p>
                </div>

                <!-- Paiements Card -->
                <div onclick="openPaiementModal()" class="bg-white rounded-2xl p-6 border border-gray-100 cursor-pointer hover:shadow-lg transform hover:-translate-y-1 transition-all duration-200">
                    <div class="flex items-center justify-between mb-4">
                        <div class="w-12 h-12 bg-green-50 rounded-xl flex items-center justify-center">
                            <i class="fas fa-credit-card text-success text-xl"></i>
                        </div>
                        <span class="text-green-600 text-sm" id="pais-percent">{{ number_format($pais_diff_percent, 1) }}%</span>
                    </div>
                    ...
                    <div class="mt-4 bg-gray-200 rounded-full h-2">
                        <div class="bg-success rounded-full h-2" style="width: {{ abs($pais_diff_percent) > 100 ? 100 : abs($pais_diff_percent) }}%"></div>
                    </div>
                    <p class="text-gray-500 text-xs mt-2" id="pais-change">{{ $pais_diff_percent >= 0 ? 'Increase' : 'Decrease' }} by {{ number_format($count_pais_filtered) }} Paiements</p>
                </div>

                <!-- Rendez-vous Card -->
                <div onclick="openAddModal()" class="bg-white rounded-2xl p-6 border border-gray-100 cursor-pointer hover:shadow-lg transform hover:-translate-y-1 transition-all duration-200">
                    <div class="flex items-center justify-between mb-4">
                        <div class="w-12 h-12 bg-green-50 rounded-xl flex items-center justify-center">
                            <i class="fas fa-calendar-check text-info text-xl"></i>
                        </div>
                        <span class="text-green-600 text-sm" id="rvs-percent">{{ number_format($rvs_diff_percent, 1) }}%</span>
                    </div>
                    ...
                    <div class="mt-4 bg-gray-200 rounded-full h-2">
                        <div class="bg-info rounded-full h-2" style="width: {{ abs($rvs_diff_percent) > 100 ? 100 : abs($rvs_diff_percent) }}%"></div>
                    </div>
                    <p class="text-gray-500 text-xs mt-2" id="rvs-change">{{ $rvs_diff_percent >= 0 ? 'Increase' : 'Decrease' }} by {{ number_format($count_rvs_filtered) }} Rendez vous</p>
                </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-100">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <button id="period_butt" class="bg-primary text-white px-3 py-1 rounded-lg text-sm">{{ ucfirst($period) }}</button>
                    </div>
                    <div class="h-80">
                                ...
                                @foreach ($latest_facs as $facture)
                                    <tr class="hover:bg-blue-50 transition-colors duration-200 cursor-pointer">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            ...
                                            @else
                                                <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">{{ ucfirst($facture->statut) }}</span>
                                            @endif
                                        </td>
                        ...
                        @foreach ($latest_rvs->take(4) as $rdv)
                            <div class="flex items-center space-x-4 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                                ...
                                <div class="text-right">
                                    <p class="font-semibold text-gray-900">{{ \Carbon\Carbon::parse($rdv->appointment_time)->format('H:i') }}</p>
    ...
    <div id="addModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white w-full max-w-2xl rounded-lg shadow-xl p-6 m-4 max-h-[90vh] overflow-y-auto">
                    ...
                    <form action="{{ route('patients.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div>
                                    <label for="situation_familiale" class="block text-sm font-medium text-gray-700 mb-1">Situation familiale</label>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="contact" class="block text-sm font-medium text-gray-700 mb-1">Téléphone principal *</label>
                                    <input type="tel" name="contact" id="contact" required value="{{ old('contact') }}" 
                                ...
                                <div>
                                    ...
                                    <input type="tel" name="telephone_secondaire" id="telephone_secondaire" value="{{ old('telephone_secondaire') }}" 
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" 
                                        placeholder="Ex: 0612345678" autocomplete="tel">
                                </div>
                                <div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                                <div>
                                    <label for="groupe_sanguin" class="block text-sm font-medium text-gray-700 mb-1">Groupe sanguin</label>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="poids" id="poids" step="0.1" min="0" max="999.99" value="{{ old('poids') }}" 
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" 
                                        placeholder="Ex: 70.5" autocomplete="off">
                                </div>
                                <div>
                                    <label for="taille" class="block text-sm font-medium text-gray-700 mb-1">Taille (cm)</label>
                                    <input type="number" name="taille" id="taille" step="0.1" min="0" max="999.99" value="{{ old('taille') }}" 
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" 
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    <label for="antecedents" class="block text-sm font-medium text-gray-700 mb-1">Antécédents</label>
            ...
            <div id="factureModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
                <div class="bg-white w-full max-w-md rounded-2xl shadow-xl p-6 m-4 max-h-[90vh] overflow-y-auto">
                    ...
                    <form action="{{ route('factures.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label for="patient_id_facture" class="block text-sm font-medium text-gray-700 mb-1">Patient *</label>
                        ...
                        <div>
                            <label for="medecin_id_facture" class="block text-sm font-medium text-gray-700 mb-1">Médecin *</label>
                        ...
                        <div>
                            <label for="secretaire_id_facture" class="block text-sm font-medium text-gray-700 mb-1">Secrétaire</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="montant_facture" class="block text-sm font-medium text-gray-700 mb-1">Montant (DH) *</label>
                                <input type="number" name="montant" id="montant_facture" step="0.01" min="0" required 
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                            </div>
                            <div>
                        ...
                        <div>
                            <label for="date_facture" class="block text-sm font-medium text-gray-700 mb-1">Date *</label>
                            <input type="date" name="date" id="date_facture" required readonly value="{{ date('Y-m-d') }}" 
            ...
            <div id="createPaiementModal" class="modal fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
                <div class="bg-white w-full max-w-lg rounded-2xl shadow-xl p-6 m-4 max-h-[90vh] overflow-y-auto">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button onclick="closePaiementModal()" class="text-gray-400 hover:text-gray-600 transition-colors">
                            <i class="fas fa-times text-lg"></i>
                        </button>
                    </div>
                    <form method="POST" action="{{ route('paiements.store') }}" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="montant_paiement" name="montant" step="0.01" min="0" required
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                            </div>
                            <div>
   ...
   <script>
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        ...(maxYValue && { max: maxYValue + (maxYValue * 0.1) })
                    },
                    x: {
                        grid: {
                            display: false
    ...
    function updateDashboardCards(data) {
        ...
        document.getElementById('facts-change').textContent = `${data.facts_diff_percent >= 0 ? 'Increase' : 'Decrease'} by ${new Intl.NumberFormat().format(data.count_facts_filtered)} Factures`;
        document.getElementById('pats-change').textContent = `${data.pats_diff_percent >= 0 ? 'Increase' : 'Decrease'} by ${new Intl.NumberFormat().format(data.count_pats_filtered)} Patients`;
        document.getElementById('pais-change').textContent = `${data.pais_diff_percent >= 0 ? 'Increase' : 'Decrease'} by ${new Intl.NumberFormat().format(data.count_pais_filtered)} Paiements`;
        document.getElementById('rvs-change').textContent = `${data.rvs_diff_percent >= 0 ? 'Increase' : 'Decrease'} by ${new Intl.NumberFormat().format(data.count_rvs_filtered)} Rendez vous`;

        // Update progress bars
        const factsBar = document.querySelector('#facts-count').closest('.bg-gradient-to-br').querySelector('.bg-white');
