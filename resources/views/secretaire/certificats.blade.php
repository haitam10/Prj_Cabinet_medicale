    ...
    <div id="templateSelectionModal"
        ...
        <div class="bg-white w-full max-w-md rounded-lg shadow-xl m-4">
            ...
            <div class="p-6">
                <div class="space-y-4">
                    ...
                    <div id="templatePreview" class="hidden bg-gray-50 p-3 rounded-lg">
                        <div class="flex items-center space-x-3">
                            <img id="templateLogo" src="/placeholder.svg" alt="Logo"
            ...
            <div class="flex justify-end space-x-3 p-6 border-t border-gray-200">
                ...
                <button onclick="proceedWithSelectedTemplate()"
                    class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
                    <i class="fas fa-print mr-2"></i>Imprimer
                </button>
            </div>
        ...
        function openPrintModal(docData) {
            ...
            fetch(`{{ url('/api/certificat') }}/${docData.id}/data`)
                ...
                .then(fullDocData => {
                    ...
                    // Now fetch available templates
                    return fetch(`{{ url('/secretaire/papier/template/certificat') }}`);
                })
                .then(response => {
                    if (!response.ok) {
                        throw new Error(`HTTP error! status: ${response.status}`);
